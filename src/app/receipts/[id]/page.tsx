  ...
  return (
    <main className="mx-auto w-full max-w-4xl space-y-4">
      ...
      <section className="rounded-2xl border border-black/10 bg-white p-4">
        ...
        <p className="mt-1 text-sm text-muted">תאריך הפקה: {fmtDate(receipt.issuedAt)}</p>
        ...
        <div className="space-y-2">
          {receipt.paymentAllocations.map((allocation) => (
            <EntityLink
              ...
              label={`${formatPatientName(allocation.session.patient.firstName, allocation.session.patient.lastName)} · ${fmtDate(allocation.session.scheduledAt)} ${fmtTime(allocation.session.scheduledAt)}`}
              variant="compact"
