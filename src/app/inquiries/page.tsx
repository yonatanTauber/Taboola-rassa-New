...
export default async function InquiriesPage() {
  ...
  const stats = {
    ...
    active: inquiries.filter((i) =>
      ["NEW", "DISCOVERY_CALL", "WAITLIST"].includes(i.status),
    ).length,
  ...
  return (
    <main className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <InquiryKpiCard label="סה״כ פניות" value={stats.total} colorClass="bg-white border-black/10 text-ink" />
        <InquiryKpiCard label="פניות פעילות" value={stats.active} colorClass="bg-amber-50 border-amber-200 text-amber-700" />
        <InquiryKpiCard label="הפכו למטופלים" value={stats.converted} colorClass="bg-emerald-50 border-emerald-200 text-emerald-700" />
        <InquiryKpiCard label="נסגרו" value={stats.closed} colorClass="bg-black/[0.03] border-black/10 text-muted" />
      </div>
